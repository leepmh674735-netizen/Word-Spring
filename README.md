...
*   **🐱 귀여운 캐릭터 테마 (Cyber-Neko Mascot)**: 마스코트 **Cyber-Neko**와 안내 말풍선, 통통 튀는 버튼 바운스 애니메이션 등 아기자기한 캐릭터 중심 디자인 적용
*   **🌈 레인보우 인터랙션 & 애니메이션 (Rainbow Animations)**: 
    *   **마우스 파티클 트레일**: 마우스를 움직일 때마다 고양이 발자국(`🐾`), 하트(`💖`), 별(`⭐`), 반짝이(`✨`), 무지개(`🌈`), 고양이(`🐱`) 파티클이 무지개색 그라데이션으로 흩날립니다.
    *   **무지개 스크롤 바**: 상단 무지개 스크롤 진행 바(Scroll Progress Bar) 및 우측 브라우저의 둥글둥글한 무지개 스크롤바 장착.
    *   **회전하는 레인보우 테두리**: 유리 카드(`glass-card`)에 마우스를 올릴 때 카드 외곽으로 원형 회전하는 레인보우 그라데이션 빛줄기 활성화.
*   **👂 귀여운 Chibi Cat Ears 데코**: 메인 카드 상단에 마우스 호버 시 쫑긋 웅크리며 반응하는 인터랙티브 고양이 귀 디자인 오너먼트 추가.
...
*   **🗄️ 하이브리드 데이터베이스 아키텍처**: MyBatis와 JPA(Hibernate)를 혼합하여 Supabase PostgreSQL 클라우드 DB 연동 및 로컬 H2 In-Memory DB 개발용 대체 설정 제공
